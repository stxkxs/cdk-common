...
public class LogGroupConstruct extends Construct {
  ...
  public LogGroupConstruct(Construct scope, Common common, LogGroupConf conf) {
    ...
    var builder = LogGroup.Builder.create(this, id("cloudwatch.log-group", conf.name())).logGroupName(conf.name())
      .logGroupClass(LogGroupClass.valueOf(conf.type().toUpperCase())).retention(RetentionDays.valueOf(conf.retention().toUpperCase()))
