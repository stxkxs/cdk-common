...
public class UserPoolTriggersConstruct {
  ...
  public UserPoolTriggersConstruct(Construct scope, Common common, IVpc vpc, UserPoolConf conf) {
    ...
    var baseLayer = LayerVersion.Builder
      .create(scope, id("layer", triggers.base().name()))
      .layerVersionName(triggers.base().name())
      .code(Code.fromAsset(triggers.base().asset()))
      .removalPolicy(triggers.base().removalPolicy())
      .compatibleArchitectures(List.of(Architecture.X86_64))
      .compatibleRuntimes(triggers.base().runtimes().stream()
        .map(r -> Runtime.Builder.create(r).build()).toList())
      .build();

    this.triggers = UserPoolTriggers.builder()
      .customMessage(maybe(scope, common, vpc, triggers.customMessage(), baseLayer))
      ...
      .postAuthentication(maybe(scope, common, vpc, triggers.postAuthentication(), baseLayer))
      .build();
  }
