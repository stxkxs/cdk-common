...
public class IdentityConstruct extends Construct {
  ...
  public IdentityConstruct(Construct scope, Common common, IdentityConf conf) {
    ...
    var hostedZone = PublicHostedZone.fromPublicHostedZoneAttributes(this, "hostedzone.lookup",
    ...
    this.hostedZoneIdentity = EmailIdentity.Builder.create(this, "hostedzone.identity").dkimIdentity(DkimIdentity.easyDkim())
      .configurationSet(this.configurationSet()).feedbackForwarding(conf.feedbackForwarding())
      .mailFromBehaviorOnMxFailure(MailFromBehaviorOnMxFailure.valueOf(conf.mxFailure().toUpperCase()))
      .mailFromDomain(conf.mailFromDomain()).identity(Identity.publicHostedZone(hostedZone)).build();

    this.emailIdentity = EmailIdentity.Builder.create(this, conf.email()).identity(Identity.email(conf.email())).build();

    this.mxInboundRecord = MxRecord.Builder.create(this, "hostedzone.mx.inbound").zone(hostedZone).recordName(conf.domain())
      .values(
  ...
  private ConfigurationSet configurationSet(ConfigurationSetConf conf) {
    var configurationSet =
      ConfigurationSet.Builder.create(this, conf.name()).configurationSetName(conf.name()).reputationMetrics(conf.reputationMetrics())
        .sendingEnabled(conf.sendingEnabled()).tlsPolicy(ConfigurationSetTlsPolicy.valueOf(conf.tlsPolicyConfiguration().toUpperCase()))
        .suppressionReasons(SuppressionReasons.valueOf(conf.suppressionReasons().toUpperCase()));

    if (conf.dedicatedIpPool().enabled()) {
      configurationSet
        .dedicatedIpPool(DedicatedIpPool.Builder.create(this, "dedicated.ip.pool").dedicatedIpPoolName(conf.dedicatedIpPool().name())
          .scalingMode(ScalingMode.valueOf(conf.dedicatedIpPool().scalingMode().toUpperCase())).build());
    }
