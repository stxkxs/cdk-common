...
public class ServiceAccountConstruct extends Construct {
  ...
  public ServiceAccountConstruct(Construct scope, Common common, ServiceAccountConf conf, ICluster cluster) {
    ...
    this.serviceAccount = ServiceAccount.Builder.create(this, conf.metadata().getName()).cluster(cluster).name(conf.metadata().getName())
      .namespace(conf.metadata().getNamespace()).labels(conf.metadata().getLabels())
      .annotations(Maps.from(conf.metadata().getAnnotations(), Map.of(AWS_ROLE_ARN, this.roleConstruct().role().getRoleArn()))).build();
  }
