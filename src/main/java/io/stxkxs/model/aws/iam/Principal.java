...
/**
 * Comprehensive IAM Principal configuration record that provides sophisticated AWS identity 
 * and access management with support for multiple principal types and complex trust relationships.
 * 
 * <p>This record serves as the foundational component for IAM role creation, policy attachment,
 * and trust relationship establishment across the entire CDK infrastructure framework, with
 * specialized support for EKS OIDC integration and composite principal scenarios.
 * 
 * <p><b>Supported Principal Types:</b>
 * <ul>
 ...
 * </ul>
 * 
 * <p><b>EKS OIDC Specialization:</b>
 * The record provides specialized methods for EKS OpenID Connect integration:
 * <ul>
 ...
 * </ul>
 * 
 * <p><b>Composite Principal Architecture:</b>
 * When multiple principals need identical permissions, the composite functionality
 * combines them into a single CompositePrincipal, reducing policy complexity and
 * improving maintainability while preserving fine-grained access control.
 * 
 ...
 * </ul>
 * 
 * <p><b>Trust Relationship Patterns:</b>
 * <pre>
 ...
 * Principal lambdaPrincipal = Principal.builder()
 *     .type(PrincipalType.SERVICE)
 *     .value("lambda.amazonaws.com")
 *     .build();
 * 
 * // EKS service account with OIDC
 * IPrincipal eksPrincipal = principal.oidcPrincipal(
 *     scope, oidcProvider, serviceAccountConfig);
 * 
 * // Composite principal for multiple access patterns
 * Principal composite = Principal.builder()
 *     .type(PrincipalType.SERVICE)
 *     .value("lambda.amazonaws.com")
 ...
 * }</pre>
 * 
 * @param type The type of principal (AWS, SERVICE, FEDERATED, ARN, STAR)
 * @param value The principal identifier (account ID, service name, ARN, etc.)
 * @param action The federated action for assume role operations
 * @param composite List of additional principals to combine with the primary principal
 * @param conditions IAM policy conditions for conditional access control
 * 
